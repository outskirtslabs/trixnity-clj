...
`ol.trixnity.media/upload` now returns `{::mx/result ::mx/progress}` for the
same reason: callers can observe upload progress while awaiting the final
uploaded media map from one operation.
