  ...
  (fn [ev]
    (when (and bot-user-id
      ...
      (cond
        ...
        (event/reaction? ev)
        (room/react! client room-id ev (event/key ev))
...
(defn- start-client! [cfg]
  (let [client-config (merge
        ...
        opened-client (.get (client/open! client-config))]
    (.get (client/start-sync! opened-client))
    (.get (client/await-running! opened-client
  ...
  (let [stored-room-id (room-state/load-room-id (:room-id-file cfg))]
    (if stored-room-id
      (do
        ...
        stored-room-id)
      (let [created-room-id (.get (room/create! client
...
(defn run-poc! []
  (let [cfg                                                                   (config/load-config)
        ...
        subscription
        (timeline/subscribe! client
                             {::mx/decryption-timeout (Duration/ofSeconds 8)}
                             (handler client room-id bot-user-id))]
    ...
    (when-let [invite-user (:invite-user cfg)]
      (try
        ...
        (.get (room/invite! client room-id invite-user))
