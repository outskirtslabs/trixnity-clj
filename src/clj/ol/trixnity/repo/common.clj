...
(def ^:private schema-statements
  [["CREATE TABLE IF NOT EXISTS account (repo_key TEXT PRIMARY KEY, payload TEXT NOT NULL) WITHOUT ROWID"]
   ...
   ["CREATE TABLE IF NOT EXISTS sticky_event (room_id TEXT NOT NULL, type TEXT NOT NULL, sender TEXT NOT NULL, sticky_key_is_null INTEGER NOT NULL, sticky_key TEXT NOT NULL, event_id TEXT NOT NULL, end_time_epoch INTEGER NOT NULL, payload TEXT NOT NULL, PRIMARY KEY (room_id, type, sender, sticky_key_is_null, sticky_key)) WITHOUT ROWID"]
   ["CREATE INDEX IF NOT EXISTS sticky_event_event_id_idx ON sticky_event (room_id, event_id)"]
   ["CREATE INDEX IF NOT EXISTS sticky_event_end_time_idx ON sticky_event (end_time_epoch)"]
